...
function EditSlider() {
  ...
  useEffect(() => {
    const fetchSliderData = async () => {
      try {
        const response = await axios.get("http://localhost:8080/admin/findslider", {
          params: { _id: id }
        });
        ...
        console.error("Error fetching slider details");
      }
    };
    if (id) {
        fetchSliderData();
  ...
  const editSliderLogic = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/admin/editslider/${id}`, formData);
      if (response.data.success) {
      ...
      <div className="content-wrapper">
        <div className="content-header">
          <div className="container-fluid">
            <h1 className="m-0">Edit Slider</h1>
          </div>
        </div>
